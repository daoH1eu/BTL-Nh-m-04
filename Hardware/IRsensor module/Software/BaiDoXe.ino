...
void loop() {
  ...
  if ((millis() - timeStart < 10000) && (millis() - timeStart > 5000)) {
   ...
   if ((millis() - timeStart < 15000) && (millis() - timeStart > 10000)) {
    Display(3);
  }
...
void check_location() {
  zone[0] = pcf[0] + pcf[1] + pcf[2] + pcf[3];
  zone[1] = pcf[4] + pcf[5] + pcf[6] + pcf[7];
  zone[2] = pcf[8] + pcf[9] + pcf[10] + pcf[11];
  zone[3] = pcf[13] + pcf[14] + pcf[15] + pcf[12];
}
